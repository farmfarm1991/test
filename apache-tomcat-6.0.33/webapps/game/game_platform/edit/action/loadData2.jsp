...
<%
	...
	String key_city = request.getParameter("City");
	String key_classifyID = request.getParameter("ClassifyID");
	System.out.println("keyword "+keyword+key_city+key_classifyID);
	String output ="";
		...
		while(rs.next()){
			inJsonObj=new JSONObject();
	 		...
	 		if(playURL.indexOf(".") < 0 || (playURL.indexOf(".") >= 0 && playURL.indexOf(".0") >= 0)){
				...
				inJsonObj.put("BookName", gameName).put("BookUrl", "http://holiyo.tn.edu.tw"+playURL).put("Count", playNum).put("AuthorName", creatorName).put("CoverUrl", "http://holiyo.tn.edu.tw/iTS5/service/file/game/tn/"+gameID+"/"+imgPath).put("qrCode", "http://www.qr-code-generator.com/phpqrcode/getCode.php?cht=qr&chl="+java.net.URLEncoder.encode("http://holiyo.tn.edu.tw"+playURL, "UTF-8")).put("Description", "").put("Grade", grade).put("Classify", classifyID).put("PublicationDate", stime).put("Country", nickName);
